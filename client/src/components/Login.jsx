  ...
  const handleGoogleSuccess = async (credentialResponse) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/auth/google-login`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ token: credentialResponse.credential }), // Send Google credential token
      });
      const data = await res.json(); // Parse the JSON response

      if (res.ok) {
        // If Google login is successful, store token and user data, then navigate to dashboard
        login(data.token, { name: data.user.name, email: data.user.email });
        setMessage('Google login successful! Redirecting to dashboard...');
        navigate('/dashboard');
      } else {
        // If Google login fails, display the error message from the backend
